...
//    }
//
//    // ========== 2. 북마크/신청 (Placeholder) 테스트 ==========
//
//    @Test
//    fun toggleBookmark_noImplementation_doesNotThrow() = runTest {
//        // 현재 ViewModel에 구현된 로직이 없으므로, 실행 시 예외가 발생하지 않는지만 확인
//        viewModel.toggleBookmark(TEST_FEED_ID)
//        advanceUntilIdle()
//        // 검증할 Repository 호출이 없으므로, Exception이 발생하지 않는 것으로 충분
//    }
//
//    @Test
//    fun applicationProgram_noImplementation_doesNotThrow() = runTest {
//        // 현재 ViewModel에 구현된 로직이 없으므로, 실행 시 예외가 발생하지 않는지만 확인
//        viewModel.applicationProgram(TEST_FEED_ID)
//        advanceUntilIdle()
//        // 검증할 Repository 호출이 없으므로, Exception이 발생하지 않는 것으로 충분
//    }
//}